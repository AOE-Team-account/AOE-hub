...
import { getFilePosts, getUserById } from "@/lib/mock-data";
import type { FileSection, MediaType } from "@/lib/types";
...
export default function FileBoardPage() {
  return (
...
function FileBoardContent() {
  ...
  return (
    <>
      ...
      {files.map((file) => {
        const author = getUserById(file.authorId);
        return (
      ...
      {files.length === 0 && <p className="muted">No files match those filters yet.</p>}
    </>
  );
}
